...
class ConcoxV5Server {
  ...
  parsePacket(buffer) {
    ...
    if (stopByte1 !== 0x0d || stopByte2 !== 0x0a) {
      log(
        `⚠️  Invalid stop bytes: 0x${stopByte1.toString(
          16
        )} 0x${stopByte2.toString(16)}`
      );
    }
  ...
  handleGPSLocation(socket, packet, clientInfo) {
    ...
    // Start(2) + Length(1) + Protocol(1) + DateTime(6) + GPSSatellites(1) + Lat(4) + Lon(4) + Speed(1) + CourseStatus(2) + MCC(2) + MNC(1) + LAC(2) + CellID(3) + ... + Serial(2) + CRC(2) + Stop(2)
    ...
    try {
      ...
      const latitudeNS = (courseStatus & 0x0400) !== 0 ? "S" : "N";
      const longitudeEW = (courseStatus & 0x0800) !== 0 ? "W" : "E";
      ...
      log(`📍 GPS Location`, {
        imei: socket.deviceImei || "unknown",
        timestamp: `${datetime.year}-${String(datetime.month).padStart(
          2,
          "0"
        )}-${String(datetime.day).padStart(2, "0")} ${String(
          datetime.hour
        ).padStart(2, "0")}:${String(datetime.minute).padStart(
          2,
          "0"
  ...
  handleAlarm(socket, packet, clientInfo) {
    ...
    try {
      ...
      const alarmType =
        alarmTypes[alarmByte1] || `Unknown (0x${alarmByte1.toString(16)})`;
